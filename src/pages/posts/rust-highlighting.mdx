...
When I first decided I wanted better highlighting for Rust, I immediately knew `rust-analyzer` had to be the way to go. It's [the official Language Server Protocol (LSP) server](https://github.com/rust-lang/rust-analyzer) for the  Rust programming language.
...
The best code highlighting on any blog can be achieved through a custom `rust-analyzer` fork. Pretty cool, right? Thanks for reading. If you have any questions, reach out on [Bluesky](https://bsky.app/profile/igamble.dev) or [Mastodon](https://mastodon.social/@igamble)!
